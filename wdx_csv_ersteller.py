...
def createCSVFileName(filename: str, dirNum: str, config: dict) -> str:
    filename = (
        f"{config["basicFileName"]}-{dirNum}_{filename[0]}_{config[filename[0]]}.csv"
    )
    return filename
...
def main(directorycontent: list) -> None:
    ...
    while validDirectory == False:
        inputDirectoryNumber: str = input(
            "Bitte geben Sie die Verzeichnisnummer der Maps an: "
        )
    ...
    for os_file in directorycontent:
        ...
        if (
            os.path.isfile(file)
            and file.suffix == ".txt"
            and file.stem.endswith("_map")
            and file.stem[0] in config
        ):
            data: pd.DataFrame = pd.read_csv(file, sep="\t", header=None)
            newFileName: str = createCSVFileName(
                file.stem, inputDirectoryNumber, config
            )
